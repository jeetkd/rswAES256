...
func EncryptRandomKeyWithPublicKey(randomKey string, publicKey rsa.PublicKey) ([]byte, error) {

	// string으로된 key를 []byte로 변환
	randKey, err := hex.DecodeString(randomKey)
	if err != nil {
		return nil, err
	}

	//공개키로 암호화
	cipherKey, err := rsa.EncryptPKCS1v15(rand.Reader, &publicKey, randKey)
	if err != nil {
		return nil, err
	}
